...
# 定义采样频率和时间间隔
fs = 60  # 采样频率为60Hz
window_size = fs  # 每秒的数据点数
# 读取CSV文件
PPG_files = ['PPG_regular_new.csv', 'PPG_lockin_new.csv']
RR_files = ['RR_regular.csv', 'RR_lockin.csv']


for i in range(2):
    input_file = data_path('Figure4/' + PPG_files[i])  # 输入文件名
    output_file = data_path('Figure4/' + RR_files[i])  # 输出文件名
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(ac1_list, label='AC_Visible')
    plt.plot(ac2_list, label='AC_NIR')
    plt.plot(dc1_list, label='DC_Visible')
    plt.plot(dc2_list, label='DC_NIR')
