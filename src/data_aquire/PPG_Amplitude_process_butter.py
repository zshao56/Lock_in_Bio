...
def detect(Amplitude):
    ...
    for i in range(num_frames):
        ...
        b = 7
        roi_coords = [
            (x0 - 10 - a, y0 + b, x0 - 5 - a, y0 + 5 + b),
            (x0 - 5 - a, y0 + b, x0 - a, y0 + 5 + b),
            (x0 - a + 5, y0 + b, x0 + 10 - a, y0 + 5 + b),
            (x0 - a, y0 + b, x0 + 5 - a, y0 + 5 + b),
        ]
...
def denoise(roi_mean):
    ...
    for i, data in enumerate(regions):
        ...
        # normalized = (rppg - np.mean(rppg)) / np.std(rppg)
        # normalized = data

        # xx = list(range(0, normalized.size))
        # pp = np.polyfit(xx, normalized, deg=16)
        # yy = normalized - np.polyval(pp, xx)
        #normalized = (yy - np.mean(yy)) / np.std(yy)
...
def main():
    ...
    task_with_folder = partial(task, out_folder_path=out_folder_path)
    with multiprocessing.Pool(processes=3) as pool:
        results = pool.map(task_with_folder, file_list)
        print(results)
def task(file_name, out_folder_path):
    ...
    interpolate_and_save_data(denoised_data, file_name, out_folder_path)

    return (file_name, True)

base_folder = '1019'
folder_path = os.path.join(r'.\PPG raw data', base_folder)
out_folder_path = os.path.join(r'.\PPG_butter', base_folder)         
